    ...
    def do_task(self, task, round_number):
        ...
        if not self.nn:
            if task_name == '2_weak_learners_validate':
                ...
                if adaboost is None:
                    adaboost = self.model_buffer.replace(self.model_buffer.get(best_model), np.log(1/alpha))#alpha)
                else:
                    adaboost.add(self.model_buffer.get(best_model),np.log(1/alpha))#alpha
