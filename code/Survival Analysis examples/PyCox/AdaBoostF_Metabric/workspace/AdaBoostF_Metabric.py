...
def train_adaboost(model, train_loader, device, optimizer, adaboost_coeff, name):
    ...
    y_train_times = y_train.iloc[:,0].values
    y_train_event =  y_train.iloc[:,1].values
    y_train = (y_train_times , y_train_event)
    ...
    verbose = False
    y_val_times = y_val.iloc[:,0].values
    y_val_event =  y_val.iloc[:,1].values
    y_val = (y_val_times,y_val_event)
    val = x_val , y_val
    ...
    ev = EvalSurv(surv,times,event, censor_surv='km')
    c_index_value = ev.concordance_td()
    event_times  = np.unique(times)  
    follow_up_time = np.max(times)
...
def validate_weak_learners(model, val_loader, device, adaboost_coeff, name):
    ...
    for idx, weak_learner in enumerate(model.get_estimators()):
        ...
        ev = EvalSurv(survs, times,event , censor_surv='km')
        c_index_value = ev.concordance_td()
        time_values = survs.iloc[:,0].index.values 
        survival_time = np.array([time_values[np.argmax(survs.iloc[:,i].values <= 0.5)] for i in survs])
        err = np.where(np.logical_or(event_indicator == 1, np.logical_and(event_indicator == 0, survival_time < time)),
               np.abs(survival_time - time),
               0)
